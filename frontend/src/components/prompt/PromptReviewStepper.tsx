...
export default function PromptReviewStepper({ response, loading, onSubmit, onRestart }: PromptReviewStepperProps) {
  ...
  const canContinue = response.turn < 3 && Boolean(response.session_id);
  return (
    <Card>
      <CardHeader><CardTitle className="text-base">审查会话</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
          <span>第 {response.turn} / 3 轮</span>
        ...
        {canContinue ? <>
          ...
          <Button className="w-full" onClick={() => { onSubmit(feedback); setFeedback(""); }} disabled={loading || !feedback.trim()}>{loading ? "更新中..." : "提交本轮意见"}</Button>
