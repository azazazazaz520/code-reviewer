...
- 首批实施范围：P0 统一设置页与安全只读闭环

## 1. 方案结论

当前应用应新增一个 /settings 页面，作为本机工作区的设置中心，统一查看外观、模型服务、代码托管、审查行为、提示词工作台、数据目录和运行诊断。配置编辑、凭据管理和 sidecar 重启分别放在后续阶段，不在 P0 只读页面中提前形成未定义的写入链路。
...
5. P0 只实现读取和展示；P1 先完成非敏感配置的持久化与生效；P1.5 再实现 safeStorage、凭据传递和 sidecar 重启。浏览器模式的 `.env` 凭据保持只读说明，不通过网页持久化。

本方案只描述实现边界、接口、交互、数据流和验收标准，不授权修改现有业务代码或提交代码。
